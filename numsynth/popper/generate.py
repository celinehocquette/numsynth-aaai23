...
class Generator:
    ...
    def __init__(self, settings, grounder, bootstrap_cons):
        ...
        encoding = []
        ALAN_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), "lp/alan.pl")
        with open(ALAN_FILE) as f:
        ...
        encoding.append(f'max_magic({settings.max_magic}).')
        encoding.append(f'max_numeric({settings.max_numeric}).')
    ...
    def parse_model(self, model):
        ...
        for atom in model:
            ...
            elif atom.name == 'magic_literal':
                rule_index = args[0].number
                predicate = args[1].name
                atom_args = args[3].arguments
                atom_args = tuple(arg_lookup[arg] for arg in atom_args)
                arity = args[2].number
                magic_literal = (predicate, atom_args, arity)
                rule_index_to_body[rule_index].add(magic_literal)

            elif atom.name == 'numerical_var':
                rule_index = args[0].number
                var_id = arg_lookup[args[1]]
                numerical[rule_index].add(var_id)
        ...
        for rule_index in rule_index_to_head:
            ...
            for (body_pred, body_args, body_arity) in rule_index_to_body[rule_index]:
                ...
                body_numerical = [i for i in range(body_arity) if rule_index in numerical and
                                  body_args[i] in numerical[rule_index]]
                body.add(Literal(body_pred, body_args, body_modes, numerical=body_numerical))
            body = frozenset(body)
            rule = head, body
            prog.append((rule))
    ...
    def build_generalisation_constraint(self, prog, rule_ordering={}, recursion=False, non_recursive=False):
        prog = list(prog)
        rule_index = {}
        literals = []
        for clause_number, rule in enumerate(prog):
            ...
            literals.append(Literal('head_literal', (clause_number, head.predicate, head.arity, tuple(vo_variable(v)
                                                                                        for v in head.arguments))))

            for body_literal in body:
                if body_literal.is_magic():
                    kind = 'magic_literal'
                # elif body_literal.is_numeric():
                #     kind = 'numerical_literal'
                else:
                    kind = 'body_literal'
                for v in body_literal.numerical:
                    literals.append(Literal('numerical_var', (clause_number, vo_variable(body_literal.arguments[v]))))
                literals.append(Literal(kind, (clause_number, body_literal.predicate, body_literal.arity,
                                                         tuple(vo_variable(v) for v in body_literal.arguments))))

            for idx, var in enumerate(head.arguments):
                literals.append(eq(vo_variable(var), idx))

            literals.append(body_magic_size_literal(clause_number, len(body)))

        if recursion and non_recursive:
            literals.append(Literal('not_recursive', ()))
    ...
    def build_specialisation_constraint(self, prog, rule_ordering={}, non_numeric=False):
        prog = list(prog)
        rule_index = {}
        literals = []
        for clause_number, rule in enumerate(prog):
            ...
            for body_literal in body:
                ...
                for v in body_literal.numerical:
                    literals.append(Literal('numerical_var', (clause_number, vo_variable(body_literal.arguments[v]))))
                numeric_size += len(body_literal.numerical)
                literals.append(Literal(kind, (clause_number, body_literal.predicate, body_literal.arity,
                                                             tuple(vo_variable(v) for v in body_literal.arguments))))

            for idx, var in enumerate(head.arguments):
                literals.append(eq(vo_variable(var), idx))
            # adding one numerical literal can only make the rule more specific while adding a second numerical
            # literal can increase the pos covered
            if non_numeric and numeric_size > 0:
                literals.append(Literal('numeric_size', (clause_number, numeric_size+1), positive=False))
...
def body_magic_size_literal(clause_var, body_size):
    return Literal('body_magic_size', (clause_var, body_size))
