...
from .core import Literal, MagicConst
...
def parse_args():
    ...
    parser.add_argument('--info', default=True, action='store_true', help='Print best programs')
    ...
    parser.add_argument('--max-magic', type=int, default=MAX_MAGIC, help=f'Maximum number of magic values in a rule'
                                                                         f' (default: {MAX_MAGIC})')
    parser.add_argument('--max-numeric', type=int, default=MAX_NUMERIC, help=f'Maximum number of numeric values in '
                                                                             f'a rule (default: {MAX_NUMERIC})')
    ...
    parser.add_argument('--magic-values', default=False, action='store_true',
                        help='Enable magic values. Can specify which variables should '
                             'be magic values with the predicates magic_evaluate/2 and magic_evaluate_type/1. If none '
                             'of these predicates is specified, any variable can be treated as a magic variable')
    parser.add_argument('--numerical-reasoning', default=False, action='store_true', help='Enable numerical reasoning.')
    parser.add_argument('--numerical-precision', type=float, default=NUMERICAL_PRECISION, help='Numerical precision for real numbers during numerical reasoning.')
...
def format_rule_debug(rule):
    head, body = rule
    head_str = ''
    if head:
        head_str = format_literal(head)
    body_str = ','.join(format_literal(literal) for literal in body if not literal)
    if not body_str:
        return f'{head_str}.'
    return f'{head_str}:- {body_str}.'
...
class Settings:
    def __init__(self, kbpath=False, info=True, debug=False, show_stats=False, bkcons=False, max_literals=MAX_LITERALS, timeout=TIMEOUT, quiet=False, eval_timeout=EVAL_TIMEOUT, max_examples=MAX_EXAMPLES, max_body=MAX_BODY, max_rules=MAX_RULES, max_vars=MAX_VARS, functional_test=False):
        ...
        self.magic_values=args.magic_values if args.magic_values else False
        self.numerical_reasoning = args.numerical_reasoning if args.numerical_reasoning else False
        ...
        self.max_numeric = args.max_numeric
        for x in solver.symbolic_atoms.by_signature('max_numeric', arity=1):
            self.max_numeric = x.symbol.arguments[0].number
        if not self.numerical_reasoning:
            self.max_numeric = 0

        types = {}
        for s in solver.symbolic_atoms.by_signature('type', arity=2):
            types[str(s.symbol.arguments[0])] = [str(c) for c in s.symbol.arguments[1].arguments]

        bounds = collections.defaultdict(dict)
        for x in solver.symbolic_atoms.by_signature('bounds', arity=3):
            b = [ele.number for ele in x.symbol.arguments[2].arguments]
            if len(b) != 2:
                raise Exception(f"bound not supported {b}")
            bounds[str(x.symbol.arguments[0])][x.symbol.arguments[1].number] = [b[0], b[1]]

        self.num_types = types
        all_types = [t for p in types for t in types[p]]

        self.precision = 1/args.numerical_precision if any([t in ['real', 'float'] for t in all_types]) else 1
        for x in solver.symbolic_atoms.by_signature('precision', arity=1):
            self.precision = x.symbol.arguments[0].number
        if any([t in ['real', 'float'] for t in all_types]):
            self.precision = 10*self.precision

        self.num_bounds = bounds
