...
#defined invented/2.
#defined lower/2.
#defined magic_eval/1.
#defined body_pred/2.
#defined numerical_pred/2.
...
magic_value_all :- not magic_value_type(_), not magic_value(_,_), magic_eval(N), N>0.
...
numeric_size(C,N) :-
   clause(C),
   #count{Var : body_literal(C,P,A,Var), numerical_pred(P,A)} = N.

:- numeric_size(C,N), max_numeric(Max), N>Max.

% force singleton for numerical variables
:-
    clause_var(C,Var),
    numerical_var(C,Var),
    #count{P,Vars : var_in_literal(C,P,Vars,Var)} != 1.

%% a variable can not have two incompatible numerical constraints with the same predicate
:-
...
%% built-in constraints for numerical predicates
:- recursive_clause(C,_,_), numerical_var(C,_).
:- body_literal(C,add,3,_), not numerical_var(C,_).
:- body_literal(C,add,3,(_,_,A)), body_literal(C,add,3,(A,_,_)).
:- body_literal(C,mult,3,(_,_,A)), body_literal(C,mult,3,(A,_,_)).


% a variable cannot be magic in numerical literals
:-
    clause(C),
    magic_var(C,V),
...
%% useless to have add with twice the same inputs
:-
    body_literal(C,add,3,Vars1),
    body_literal(C,add,3,Vars2),
    not Vars1 = Vars2,
    var_pos(Var1,Vars1,0),
    var_pos(Var2,Vars1,1),
    var_pos(Var1,Vars2,0),
    var_pos(Var2,Vars2,1).
...
%% for magic variables in the head
all_var(C,Var) :- body_var(C,Var).
all_var(C,Var) :- magic_var(C,Var).


%% ELIMINATE SINGLETONS APART FROM NUMERICAL VARIABLES
:-
    not allow_singletons,
    clause_var(C,Var),
    not numerical_var(C,Var),
    #count{P,Vars : var_in_literal(C,P,Vars,Var)} == 1.
%% OTHER VARIABLES IN NUMERICAL LITERALS CANNOT BE SINGLETONS
:-
    body_literal(C,P,A,Vars),
    numerical_pred(P,A),
    var_in_literal(C,P,Vars,Var),
    not numerical_var(C,Var),
    #count{P1,Vars1 : var_in_literal(C,P1,Vars1,Var)} == 1.
...
direction_(magic,0,out) :- direction(_,_).
