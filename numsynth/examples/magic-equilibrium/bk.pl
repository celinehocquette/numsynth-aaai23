...
my_mult(A,B,C) :- nonvar(A), nonvar(C),\+(A=0.0), B is C/A.
my_mult(A,B,C) :- nonvar(C), nonvar(B),\+(B=0.0), A is C/B.

my_div(A,B,C) :- nonvar(A), nonvar(B), nonvar(C),\+(B=0.0),!, precision(P), C1 is A/B-10**(-P), C2 is A/B+10**(-P), C1=<C, C=<C2.
my_div(A,B,C) :- nonvar(A), nonvar(B), \+(B=0.0), C is A/B.
my_div(A,B,C) :- nonvar(A), nonvar(C), \+(C=0.0), B is A/C.
my_div(A,B,C) :- nonvar(C), nonvar(B), \+(B=0.0), A is B*C.

my_substract(A,B,C) :- nonvar(A), nonvar(B), C is A-B.
my_substract(A,B,C) :- nonvar(A), nonvar(C), B is A-C.
my_substract(A,B,C) :- nonvar(C), nonvar(B), A is B+C.
...
my_square(A,B) :-  \+(nlist(A)), A < 10**6, float(A), B is A*A.
